...
<tr><td>Nombre</td> <td>Tipo</td> <td>Lugar</td><td>Alojamiento(si/no)</td><td>Comidas(si/no)</td><td>Precio por persona</td><td>Fecha inicio</td><td>Fecha finalización</td></tr>
<?php 
$bd=mysqli_connect("localhost","id20911504_admin","#Alfanmc02","id20911504_turismo");
$ins=mysqli_query($bd,"SELECT ac.ID, ac.NOMBRE, it.NOMBRE_ACTI, ac.LUGAR, ac.ALOJAMIENTO, ac.COMIDAS, ac.PRECIO, ac.FECHA_IN, ac.FECHA_FIN  FROM  actividad ac inner join tipos_actidades it on it.ID=ac.TIPO_ACTIVIDAD");
while($fila=mysqli_fetch_assoc($ins)){
extract($fila);
echo "<tr><td>$NOMBRE</td><td>$NOMBRE_ACTI</td><td>$LUGAR</td><td>$ALOJAMIENTO</td><td>$COMIDAS</td><td>$PRECIO</td><td>$FECHA_IN</td><td>$FECHA_FIN</td><td><a href='modificaacti.php?id=$ID'><button type='button' class='btn btn-primary'>Modificar</button></a></td><td><a href='eliminaracti.php?id=$ID'><button type='button' class='btn btn-primary'>Eliminar</button></a></td></tr>";
}
?>
</table>
<a href="nuevotipo.php"><button type='button' class='btn btn-primary'>Añadir nueva categoria</button></a>
<a href="creaacti.php"><button type='button' class='btn btn-primary'>Añadir nueva actividad</button></a>
</div>
<footer>
</footer>
