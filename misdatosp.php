...
$bd=mysqli_connect("localhost","id20911504_admin","#Alfanmc02","id20911504_turismo");
 if($_SESSION["usuario"]["correo"]!="admin@gmail.com"){
$ins=mysqli_query($bd,"UPDATE `usuario` SET `NOMBRE_APELLIDOS`='$nombre',`CORREO`='$correo',`TELEFONO`=$telefono,`password`='$contraseña' WHERE DNI ='$dni'");}
else{
    $ins=mysqli_query($bd,"UPDATE `usuario` SET `NOMBRE_APELLIDOS`='$nombre',`TELEFONO`=$telefono,`password`='$contraseña' WHERE DNI ='$dni'");
    
}
if($ins){
echo "<h2>Se ha cambiado todos los datos correctamente</h2>";
}
else{
echo "<h2>No se han podido cambiar los datos</h2>";}
?></body></hmtl>