...
$ins=mysqli_query($bd,"SELECT * FROM actividad where ID=$id");
if($fila=mysqli_fetch_assoc($ins)){
extract($fila);
echo "
<h1> para la actividad $NOMBRE </h1>
<div class='container formulario'>
<form action='modificaactip.php?id=$ID' method='POST'>
  <div class='mb-3'>
  <label for='nombreacti' class='form-label'>nombre de la actividad</label>
    <input type='text' class='form-control' id='nombreacti' value='$NOMBRE'  name='nombre'>
  </div>
   <div class='mb-3'>
  <label for='lugar' class='form-label'>nombre del lugar</label>
    <input type='text' class='form-control' id='lugar' value='$LUGAR'  name='lugar'>
  </div>
  <div class='mb-3'>
  <label for='precio' class='form-label'>precio por persona</label>
    <input type='number' class='form-control' id='precio' value='$PRECIO'  name='precio'>
  </div>
   <div class='mb-3'>
  <label for='fechain' class='form-label'>fecha de inicio</label>
    <input type='date' class='form-control' id='fechain' value='$FECHA_IN'  name='fechain'>
  </div>
     <div class='mb-3'>
  <label for='fechafin' class='form-label'>fecha de finalización</label>
    <input type='date' class='form-control' id='fechafin' value='$FECHA_FIN'  name='fechafin'>
  </div>
  <div class='mb-3'>
 ...
 <div class='mb-3'> <label for='comidas' class='form-label'> ¿tiene comidas?</label><select name='comidas' id='comidas'>";
  ...
  echo "</select></div> <div class='mb-3'>  <label for='tipo' class='form-label'> tipo de la actividad</label> <select name='tipo' id='tipo'>";
  $res=mysqli_query($bd,"SELECT * FROM `tipos_actidades`");
while($filas=mysqli_fetch_array($res)){
	if($filas[0]=$TIPO_ACTIVIDAD){
	echo"<option value='$filas[0]' selected>$filas[1]</option>";
	}
	else{
