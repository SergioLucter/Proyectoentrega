...
$ins=mysqli_query($bd,"SELECT * FROM usuario where correo ='".$_SESSION["usuario"]["correo"]."'");
if($fila=mysqli_fetch_assoc($ins)){
extract($fila);
echo "
<h1> para la cuenta con DNI $DNI</h1>
<div class='container formulario'>
<form action='misdatosp.php?dni=$DNI' method='POST'>
  <div class='mb-3'>";
  if($CORREO!="admin@gmail.com"){
  echo "  <label for='Email1' class='form-label'>Correo</label>
    <input type='email' class='form-control' id='Email1' value='$CORREO'  name='correo'>
  </div>";}
  else{
 echo "  <label for='Email1' class='form-label'>Correo</label>
    <input type='email' class='form-control' id='Email1' value='$CORREO'  name='correo' disabled>
  </div>";  
  }
  echo "
   <div class='mb-3'>
  ...
  <div class='mb-3'>
    ...
    <input type='text' class='form-control' id='nombre' value='$NOMBRE_APELLIDOS'  name='nombre'>
  </div>
  <div class='mb-3'>
    <label for='Password1' class='form-label'>Contraseña nueva</label>
    <input type='password' class='form-control' id='Password1' value='$password' name='contraseña'>
  </div>
