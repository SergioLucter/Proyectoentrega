...
extract($_POST);
$bd=mysqli_connect("localhost","id20911504_admin","#Alfanmc02","id20911504_turismo");
$ins=mysqli_query($bd,"SELECT 	usuario.DNI,actividad_usuario.preciototal,actividad_usuario.personas,actividad.ID,actividad.LUGAR, actividad.NOMBRE,actividad.FECHA_IN,actividad.FECHA_FIN ,actividad.COMIDAS ,actividad.PRECIO, actividad.ALOJAMIENTO,tipos_actidades.NOMBRE_ACTI   FROM usuario inner join actividad_usuario on usuario.DNI=actividad_usuario.DNI inner join actividad on actividad.id=actividad_usuario.id inner join tipos_actidades on tipos_actidades.id=actividad.tipo_actividad where usuario.DNI='$usuario' order by usuario.DNI");
if(mysqli_num_rows($ins)>0){
    echo "<h1>Listado de actividades en las que el usuario elegido se encuentra inscrito</h1><div class='container informacion'>
<table class='table table-bordered'>
<tr><td>Nombre</td> <td>Tipo</td> <td>Lugar</td><td>Alojamiento(si/no)</td><td>Comidas(si/no)</td><td>Precio Total</td><td>Nº Personas</td><td>Fecha inicio</td><td>Fecha finalización</td></tr>";
while($fila=mysqli_fetch_assoc($ins)){
extract($fila);
echo "<tr><td>$NOMBRE</td><td>$NOMBRE_ACTI</td><td>$LUGAR</td><td>$ALOJAMIENTO</td><td>$COMIDAS</td><td>$preciototal</td><td>$personas</td><td>$FECHA_IN</td><td>$FECHA_FIN</td><td><a href='eliminarelacion.php?id=$ID&dni=$DNI'><button type='button' class='btn btn-primary'>Eliminar</button></a></td></tr>";
}
echo "</table>
<a href='listadousuarioactividadpdf.php?usuario=$usuario'><button type='button' class='btn btn-primary'>Ver en pdf</button></a>   <a href='selectusuarios.php'><button type='button' class='btn btn-primary'>seleccionar otro usuario</button> </a>
</div>";
}
else{
    echo"<h2>El usuario seleccionado no se encuentra inscrito en ninguna actividad</h2>";
